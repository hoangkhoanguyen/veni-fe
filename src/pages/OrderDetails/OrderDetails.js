...
export const OrderDetails = () => {
    ...
    useEffect(()=>{
        setTimeout(()=>{
            let total = 0
            let arrAmount = document.querySelectorAll('.order-details-page table .amount')
            console.log(quantity)
            arrAmount && arrAmount.forEach((item,index)=>{
    ...
    <div className='order-details-page'>
      ...
      <h3>My order details</h3>
        <div className="product-list-container">
          ...
          {orderInfo && <p className='summary-area'>Total: {isReOrder?totalPrice:orderInfo.totalPrice} $</p>}
          <div className="button-area">
          {!isReOrder&&<button  onClick={handleClickReOrderBtn}>Re-order</button>}
          {isReOrder&& totalPrice>0 &&<button  onClick={handleClickOrderBtn}>Order</button>}
          {isReOrder&& totalPrice==0 && <button className='disable'>Order</button>}
