...
export const Login = () => {
    ...
    const handleLinkGoogleSuccess = async (googleData) => {
        if (googleData && googleData.tokenId) {
            try {
                let result = await userService.loginWithGoogle({tokenId:googleData.tokenId})
                console.log(result)
    ...
    const responseFacebook = async (response) => {
        try {
            let result = await userService.loginWithFacebook({accessToken:response.accessToken})
            console.log(result)
            if(result && result.errCode === 0) {
                let data = result.data
                dispatch(updateUserInfo({
    ...
    return (
        <div className='login-container'>
                ...
                <div className='info-content' >
                    ...
                    <div className='row'>
                        <div className="col-12 mb-3">
                            <button className='button-outline' onClick={handleClickLogin}>Log in</button>
                        </div>
                        <div className="col-12 mb-3">
            ...
            <p>You have no account? <a className='register' href={path.REGISTER}>Register now</a></p>
